        ...
        <Route
          ...
          element={
            <DashboardShell
              ...
              loadClaimEvidenceAsset={async (claimId, evidenceId) => getClaimEvidenceAssetUrl(session.token, claimId, evidenceId)}
...
function ClaimDetailPage(props: {
  ...
  useEffect(() => {
    ...
    let active = true;
    props.loadClaimEvidenceAsset(claim.id, evidence[0].id)
      .then((url) => {
        if (active) setEvidencePreviewUrl(url);
      })
      .catch(() => {
        if (active) setEvidencePreviewUrl(null);
      });
    return () => { active = false; };
  }, [claim?.id, evidence, props.loadClaimEvidenceAsset]);
  ...
  const mileageTotal = Number(((Number(claim.mileageTotalMiles ?? 0)) * (Number(claim.mileageRate ?? 0))).toFixed(2));
  const saveMileageDetails = async () => {
    if (claim.status !== "pending") {
      setError("Only pending mileage claims can be changed.");
      return;
    }
    ...
    if (!Number.isFinite(Number(claim.mileageTotalMiles)) || Number(claim.mileageTotalMiles) <= 0 || !Number.isFinite(Number(claim.mileageRate)) || Number(claim.mileageRate) < 0) {
      setError("Enter a valid mileage total and rate before saving.");
      return;
    }
    ...
    try {
      ...
      setClaim(saved);
      setFeedback("Mileage claim details saved.");
    } catch (saveError) {
      setError(saveError instanceof Error ? saveError.message : "Could not save mileage claim details.");
    } finally {
        ...
        <div className="workspace-split">
          <section className="panel viewer-panel">
            <div className="panel-heading">
              ...
              <div className="toolbar">
                {evidence[0] ? <span>{evidence[0].sourceFilename}</span> : null}
            ...
            {evidencePreviewUrl ? (
              <div className="document-image-frame mileage-evidence-frame"><img className="document-image" src={evidencePreviewUrl} alt={evidence[0]?.sourceFilename ?? "Journey proof"} /></div>
            ) : (
              <div className="document-placeholder"><strong>Mileage claim</strong><p>Add a photo or image of journey evidence when available. Proof images remain separate from receipts and do not change the claim total.</p></div>
            )}
            {evidence.length > 1 ? <ul className="claim-evidence-list">{evidence.slice(1).map((item) => <li key={item.id}>{item.sourceFilename}</li>)}</ul> : null}
          </section>

          <section className="panel editor-panel">
            <div className="panel-heading"><h2>Mileage claim review</h2><span>Organisation #{claim.organisationId}</span></div>
            <div className="form-grid">
    ...
    {status?.available && status.connected && settings && referenceData ? <>
      ...
      <section className="panel">
        ...
        <div className="form-grid">
          ...
          <label>Company-card bank account<select value={settings.companyCardBankAccountCode ?? ""} onChange={(event) => setSettings({ ...settings, companyCardBankAccountCode: event.target.value || null })}><option value="">Publish company-card costs as bills</option>{referenceData.bankAccounts.map((account) => <option key={account.accountId} value={account.code}>{account.code} — {account.name}</option>)}</select><span className="field-hint">Choose a bank account to publish company-card costs as Xero Spend Money transactions.</span></label>
...
function SettingsPage(props: {
  ...
  const [teamSectionError, setTeamSectionError] = useState<string | null>(null);
  ...
  useEffect(() => {
    void refreshTeam().catch((teamError) => {
      setTeamSectionError(teamError instanceof Error ? teamError.message : "Could not load team settings.");
    });
  }, [props.session.token]);
  ...
  return (
    <section className="panel-stack settings-shell">
      ...
      <div className="team-management-section">
        ...
        {departmentError ? <div className="error-banner">{departmentError}</div> : null}
        {departmentFeedback ? <div className="success-banner">{departmentFeedback}</div> : null}
        <div className="toolbar">
          <input
          ...
          <button
            ...
            onClick={async () => {
              ...
              try {
                ...
                setDepartmentFeedback("Department added.");
                void refreshTeam().catch((teamError) => {
                  setTeamSectionError(teamError instanceof Error ? teamError.message : "Could not refresh team settings.");
                });
              } catch (departmentError) {
                setDepartmentError(departmentError instanceof Error ? departmentError.message : "Could not add the department.");
              } finally {
                setTeamBusy(false);
              }
            }}
                  ...
                  onChange={async (event) => {
                    ...
                    try {
                      ...
                      setTeamMembers((current) => current.map((item) => item.id === member.id
                        ...
                        : item));
                      setTeamFeedback("Team member department updated.");
                      void refreshTeam().catch((teamError) => {
                        setTeamSectionError(teamError instanceof Error ? teamError.message : "Could not refresh team settings.");
                      });
                    } catch (assignmentError) {
                      setTeamSectionError(assignmentError instanceof Error ? assignmentError.message : "Could not update the department.");
                    } finally {
                      setTeamBusy(false);
                    }
                  }}
        ...
        <div className="site-footer">
          <div className="site-footer-brand">
            ...
            <a
              ...
              href="https://www.facebook.com/profile.php?id=61594025846475"
              target="_blank"
              rel="noreferrer"
              aria-label="Follow Exdox on Facebook"
              title="Follow Exdox on Facebook"
            >
